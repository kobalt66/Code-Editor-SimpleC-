...
class Error:
    ...
    def throw(self):
        return f'{self.errorType} in {self.fileName} (line: {self.position.ln}, {self.position.col}):\n\t\t{self.message}'
...
class compile2Csharp:
    ...
    def getVarValue(self, var):
        ...
        if var.value.type == FLT:
            return str(value) + 'f'
        elif var.value.type in [STR, CHR]:
...
def run(projectdir):
    ...
    return f'Successfully compiled /{projectdir}!', ''

def runScript(fn, code):
    # Reset data
    global masterscript, metacode, usedLibs, outputdir
    masterscript = MasterScript([])
    usedLibs = []
    ...
    ast = parser.parse()
    if ast.error:
        return ast.error.throw()

    # Compiling the script
    compiler = compile2Csharp(masterscript, outputdir)
    error = compiler.compile()
    if error:
        return error.throw()
